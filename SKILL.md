...
The following files give an example of how to build a project with JimmyTorch:
...
### Directory Structure For Datasets Folder
```
ProjDir/Datasets/
├── __init__.py   ---> Package initialization
├── MyDataset1.py   ---> Impl of MyDataset1 class
├── MyDataset2.py   ---> Impl of MyDataset2 class
└── ...
```
...
```python
# in Datasets/MyDataset1.py
from JimmyTorch.Datasets import *

class MyDataset1(JimmyDataset):
    ...
    def __getitem__(self, idx):
        ...
        # Must return a dict containing **ALL** available data types
        return {
            'data_A': batch_data_A,
            'data_B': batch_data_B,
            ... # other data types
...
### Directory Structure For Models Folder
```
ProjDir/Models/
├── __init__.py   ---> Package initialization
├── MethodName1/
│   ├── __init__.py   ---> Model package initialization
│   ├── ModelVariant1.py   ---> Impl of ModelVariant1
│   ├── ModelVariant2.py   ---> Impl of ModelVariant2
|   ├── ...
...
class MyModel(JimmyModel):
    ...
    def trainStep(self, batch: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
        # Some pre-processing if needed
        with getAutoCast(batch['data'], self.mixed_precision):
            output1, output2, ... = self(batch['data'])
            mae = self.mae_func(output1, batch['target1'])
            mse = self.mse_func(output2, batch['target2'])
            ...
            loss = mae + mse + ...  # combine losses as needed
    ...
    def testStep(self, batch: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
        with torch.no_grad():
            output = self(batch['data'])
            loss = self.loss_fn(output, batch['target']).item()
            mae = nn.L1Loss()(output, batch['target']).item()
...
- **Loss Names**: `train_loss_names` and `eval_loss_names` must be defined, and they must include a main loss.
...
### Directory Structure For TrainEvalTest Folder And Entry Scripts
```
ProjDir/TrainEvalTest/
...
### Usage
You should refer to `JimmyExperiment.py` and `JimmyTrainer.py` for example implementations. Each project that uses JimmyTorch should implement its own `Experiment` and `Trainer` classes. They should look very similar to the ones in `JimmyExperiment.py` and `JimmyTrainer.py`.

### Contents of TrainEvalTest
