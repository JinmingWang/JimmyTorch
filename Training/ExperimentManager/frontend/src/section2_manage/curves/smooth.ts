...
export function smoothSeries(points: ScalarPoint[], factor: number): ScalarPoint[] {
  if (factor <= 0 || points.length === 0) return points;
  const f = Math.min(0.99, factor);
  const out: ScalarPoint[] = new Array(points.length);
...
export function computeRange(
  seriesList: ScalarPoint[][],
  override: [number | null, number | null],
  axis: "x" | "y",
): [number, number] {
