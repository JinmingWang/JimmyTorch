...
export function parseModelArchitecture(source) {
    ...
    const stack = [{ indent: rootEntry.indent, node: deepestNode(root) }];
...
function parseOpenHeader(text) {
    ...
    return names.reverse().reduce((child, name) => ({ name, parameters: "", children: child ? [child] : [] }), undefined);
}
function parseInlineChildren(inner) {
...
function parenthesisBalance(source) {
    let balance = 0;
...
function toNode(expression, key, parentPath, id) {
