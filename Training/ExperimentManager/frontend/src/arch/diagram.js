...
export class ModelDiagram {
    ...
    connector(parent, child, isEntering, depth, siblingIndex) {
        ...
        group.append(element("path", { d: `M ${startX} ${startY} C ${midpoint} ${startY}, ${midpoint} ${endY}, ${endX} ${endY}` }), element("circle", { cx: String(startX), cy: String(startY), r: "4" }), element("circle", { cx: String(endX), cy: String(endY), r: "4" }));
        return group;
    }
    card(position, isEntering) {
        ...
        const activate = () => this.activate(node, depth, hasChildren, isExpanded);
    ...
    activate(node, depth, hasChildren, isExpanded) {
        if (this.isTransitioning)
            return;
        this.selectedId = node.id;
        if (hasChildren && depth > 0) {
            if (isExpanded) {
                for (const key of [...this.expandedByDepth.keys()])
                    if (key >= depth)
                        this.expandedByDepth.delete(key);
    ...
    animateCollapse(depth) {
        this.isTransitioning = true;
        this.target.querySelectorAll(`.module-card[data-depth]:not([data-depth="${depth}"])`).forEach((card) => {
            if (Number(card.dataset.depth) > depth)
                card.classList.add("is-leaving");
        });
        this.target.querySelectorAll(`.connector[data-depth]:not([data-depth="${depth}"])`).forEach((connector) => {
            if (Number(connector.dataset.depth) > depth)
...
function textElement(name, attributes, content) {
...
function moduleCategory(name) {
    if (name === "Sequential")
        return "sequential";
    if (/^(Linear|MHSA)$/.test(name))
        return "linear";
    if (/Dropout/.test(name))
        return "dropout";
    if (/Norm/.test(name))
        return "normalization";
    if (/Conv/.test(name))
        return "convolution";
    if (/^(SiLU|ReLU|GELU|Sigmoid|Tanh)$/.test(name))
        return "activation";
    if (/Loss$/.test(name))
        return "loss";
    if (/^(Transpose|Permute|Flatten|Unflatten|Rearrange)$/.test(name))
        return "shape";
    return "custom";
}
function cardMeta(node) {
    if (node.children.length > 0)
        return `${node.children.length} module${node.children.length === 1 ? "" : "s"}`;
